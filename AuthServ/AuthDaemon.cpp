...
void dm_auth_addacct(Auth_AddAcct* msg)
{
    ...
    PGresult* result = DS::PQexecVA(s_postgres, 0,
    ...
    if (PQntuples(result) == 0) {
        ...
        PGresult* result = DS::PQexecVA(s_postgres, 0,
                ...
                "    VALUES ($1, $2, $3, 0, 1)"
                "    returning idx",
                gen_uuid().toString(), pwHash.toString(), msg->m_acctInfo.m_acctName);
        if (PQresultStatus(result) != PGRES_TUPLES_OK) {
...
void dm_auth_login(Auth_LoginInfo* info)
{
    ...
    // Get list of players
    result = DS::PQexecVA(s_postgres, 0,
...
void dm_auth_disconnect(Auth_ClientMessage* msg)
{
    ...
    if (client->m_player.m_playerId) {
        ...
        PGresult* result = DS::PQexecVA(s_postgres, 0,
                ...
                "    RETURNING idx",
                DS::Vault::e_NodePlayer, client->m_player.m_playerId);
        if (PQresultStatus(result) != PGRES_TUPLES_OK) {
...
void dm_auth_deletePlayer(Auth_PlayerDelete* msg)
{
    ...
    result = DS::PQexecVA(s_postgres, 0,
                          ...
                          "    WHERE \"PlayerIdx\"=$1", msg->m_playerId);
...
void dm_auth_createAge(Auth_AgeCreate* msg)
{
    ...
    PGresult* result = DS::PQexecVA(s_postgres, 0,
            ...
            msg->m_age.m_ageId.toString(), DS::Vault::e_NodeAge);
    if (PQresultStatus(result) != PGRES_TUPLES_OK) {
        fprintf(stderr, "%s:%d\n    Postgres SELECT error: %s\n",
                __FILE__, __LINE__, PQerrorMessage(s_postgres));
        SEND_REPLY(msg, DS::e_NetInternalError);
    ...
    if (PQntuples(result) == 1) {
        ...
        PGresult* result = DS::PQexecVA(s_postgres, 0,
                "SELECT idx FROM vault.\"Nodes\""
                "   WHERE \"Uuid_1\"=$1 AND \"NodeType\"=$2",
                msg->m_age.m_ageId.toString(), DS::Vault::e_NodeAgeInfo);
        if (PQresultStatus(result) != PGRES_TUPLES_OK) {
            fprintf(stderr, "%s:%d\n    Postgres SELECT error: %s\n",
            __FILE__, __LINE__, PQerrorMessage(s_postgres));
            SEND_REPLY(msg, DS::e_NetInternalError);
...
void dm_auth_findAge(Auth_GameAge* msg)
{
    ...
    PGresult* result = DS::PQexecVA(s_postgres, 0,
            ...
            "    WHERE \"AgeUuid\"=$1", msg->m_instanceId.toString());
    if (PQresultStatus(result) != PGRES_TUPLES_OK) {
        fprintf(stderr, "%s:%d:\n    Postgres SELECT error: %s\n",
                __FILE__, __LINE__, PQerrorMessage(s_postgres));
        PQclear(result);
    ...
    if (PQntuples(result) == 0) {
        ...
        result = DS::PQexecVA(s_postgres, 0,
                ...
                msg->m_instanceId.toString(), msg->m_name);
        if (PQresultStatus(result) != PGRES_TUPLES_OK) {
            fprintf(stderr, "%s:%d:\n    Postgres INSERT error: %s\n",
                    __FILE__, __LINE__, PQerrorMessage(s_postgres));
            PQclear(result);
    ...
    result = DS::PQexecVA(s_postgres, 0,
                ...
                "    WHERE \"NodeType\"=$1 AND \"Uint32_1\"=$2"
                "    RETURNING idx", DS::Vault::e_NodePlayerInfo,
                reinterpret_cast<AuthServer_Private*>(msg->m_client)->m_player.m_playerId,
                ageDesc, msg->m_instanceId.toString());
    if (PQresultStatus(result) != PGRES_TUPLES_OK) {
        fprintf(stderr, "%s:%d:\n    Postgres UPDATE error: %s\n",
                __FILE__, __LINE__, PQerrorMessage(s_postgres));
        // This doesn't block continuing...
...
{
    PGresult* result = DS::PQexecVA(s_postgres, 0, "UPDATE vault.\"Nodes\""
                            "    SET \"ModifyTime\"=$1, \"Int32_2\"=1"
                            "    WHERE idx=$2 AND \"NodeType\"=$3",
...
void dm_auth_update_globalSDL(Auth_UpdateGlobalSDL* msg)
{
    ...
    for (size_t i = 0; i < state.data()->m_simpleVars.size(); ++i) {
        ...
        if (var->descriptor()->m_name == msg->m_variable) {
            ...
            PGresult* result = DS::PQexecVA(s_postgres, 0, "UPDATE vault.\"GlobalStates\""
                                            "    SET \"SdlBlob\" = $2"
                                            "    WHERE \"Descriptor\" = $1",
                                            msg->m_ageFilename,
